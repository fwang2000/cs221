...
template <typename T>
void BinaryTree<T>::printNode(Node* node) const {
...
template <typename T>
int BinaryTree<T>::sumNodes(Node * node, int sumSoFar) const {
	...
	return sumNodes(node->left, sumSoFar) + sumNodes(node->right, sumSoFar) + sumSoFar;
