...
private:
   ...
   */
   ...
   void renderHelper(const Node * node, PNG & img);

   int pruneSizeHelper(twoDtree::Node * node, int tol);

   void pruneHelper(twoDtree::Node * node, int tol);

   bool shouldPrune(twoDtree::Node * node, int tol, RGBAPixel a);

   int countNodes(twoDtree::Node * node);

   void clearHelper(twoDtree::Node * node);

   Node * copyHelper(twoDtree::Node *other);
};
